...
public class DataDrivenExcelwithArrayList {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < NoofSheets; i++) {
			if (workbook.getSheetName(i).equals("testData")) {
				...
				while (Coulmncellorbox.hasNext()) {
					System.out.println("====checking excel headers======");

					Cell value = Coulmncellorbox.next();
					if (value.getStringCellValue().equalsIgnoreCase("Testcase")) {
						column = k;// desired column
						System.out.println("===printing column index=======");
				...
				while (rows.hasNext()) {
					...
					if (r.getCell(column).getStringCellValue().equalsIgnoreCase("Purchase")) {
						...
						//	System.out.println("===Based on header fetching data from desired row=======");
		...
		System.out.println(arrayListfromExcel);
	}
