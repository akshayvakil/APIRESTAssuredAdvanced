...
//response = response of login service needed for actual login
		...
		// print all course Titles in WebAutomation json a

		List<WebAutomation> webautomation = gc.getCourses().getWebAutomation();
		for (int i = 0; i < webautomation.size(); i++) {
			System.out.println("inside for loop");
			System.out.println("web automation course ' " + webautomation.get(i).getCourseTitle()
					+ "' and its price is Rs-> " + webautomation.get(i).getPrice());
